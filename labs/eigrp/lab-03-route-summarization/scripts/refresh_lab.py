...
class LabRefresher:
    ...
    def push_config(self, host, port, config_file):
        ...
        try:
            ...
            # Phase 1: Default all interfaces and remove routing protocols
            cleanup = self._parse_cleanup_commands(config_file)
            for cmd in cleanup:
                tn.sendall(cmd.encode('ascii') + b"\r\n")
                time.sleep(0.3)

            # Phase 2: Push initial config
            with open(config_file, 'r') as f:
                for line in f:
...
if __name__ == "__main__":
    devices = [
        ...
        ("R7", 5007, "initial-configs/R7.cfg")
    ]
    refresher = LabRefresher(devices)
    refresher.run()
    print("\nLab 03 has been refreshed to the initial config state.")
